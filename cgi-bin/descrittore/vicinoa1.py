...
import json, urllib2, math, csv, tempfile, codecs, rdflib, rdfextras, threading, time
...
def piglia(host,listout,radlatA,radlonA,req,MAX,DISTANZA,CATEGORY):
	...
	if 'application/json' in smart_str(r.info().getheader('Content-Type')):
		...
		for id, value in locations.items():
			...
			a = round((RT * math.acos(math.sin(radlatA) * math.sin(radlatB) + math.cos(radlatA) * math.cos(radlatB) * math.cos(radlonA - radlonB))),1)
			if (DISTANZA == 'none') or (a < float(DISTANZA)):
