    ...
    if MAX != 'none':
        if categoria == 'all' or categoria == 'none':
            if DISTANZA != 'none':
                ...
                cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT %s;",(latA,lonA,latA,MAX))
        else:
            if DISTANZA != 'none':
