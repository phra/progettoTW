...
#	listout.append((id, lat, long, category, name, opening, closing, address, tel, '', '', a))
    ...
    if MAX != 'none':
        if categoria == 'all' or categoria == 'none':
            if DISTANZA != 'none':
                cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a WHERE (6372 * acos(cos(radians(latitude)) * cos(radians(%s) ) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) < %s ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT %s;",(latA,lonA,latA,latA,lonA,latA,DISTANZA,MAX))
            else:
                cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT %s;",(latA,lonA,latA,latA,lonA,latA,MAX))
        else:
            if DISTANZA != 'none':
                cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a WHERE category = %s AND (6372 * acos(cos(radians(latitude)) * cos(radians(%s) ) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) < %s ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT %s;",(categoria,latA,lonA,latA,latA,lonA,latA,DISTANZA,MAX))
            else:
                cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a WHERE category = %s ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT %s;",(categoria,latA,lonA,latA,MAX))
    else:
        if categoria == 'all' or categoria == 'none':
            cur.execute("((SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a)) ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(10)) * cos(radians(40) - radians(longitude)) + sin(radians(latitude)) * sin(radians(10)))) LIMIT 10;",(latA,lonA,latA))
        else:
            cur.execute("SELECT * FROM (SELECT * FROM locations UNION ALL SELECT * FROM waiting) AS a WHERE category = %s ORDER BY (6372 * acos(cos(radians(latitude)) * cos(radians(%s)) * cos(radians(%s) - radians(longitude)) + sin(radians(latitude)) * sin(radians(%s)))) LIMIT 10;",(categoria,latA,lonA,latA))
